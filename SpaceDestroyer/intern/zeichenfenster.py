...
def load_settings():
    ...
    data["resolution"] = normalize_resolution(data["resolution"])
...
class SingletonMeta(type):
    """
    Hilfsklasse zur Realisierung des Singleton-Musters, damit zu jeder Zeit nur ein einziges Objekt der Klasse
    Zeichenfenster existiert.
    """
...
class Zeichenfenster(BaseClassThread, metaclass=SingletonMeta):
    ...
    def __init__(self):
        ...
        self.FENSTERBREITE, self.FENSTERHOEHE = self.data["resolution"]
        self.FPS = 4
    ...
    def run(self):
        ...
        pygame.init()
        self.data = load_settings()
        groesse = self.data["resolution"]
        self.fenster = pygame.display.get_surface()
        if self.fenster is None:
            # Überprüfen ob fullscreen ausgewählt wurde
            if self.fullscreen:
                ...
                self.FENSTERBREITE, self.FENSTERHOEHE = self.fenster.get_size()
                ...
                self.FENSTERBREITE, self.FENSTERHOEHE = groesse
            pygame.display.set_caption("Zeichenfenster")
        else:
            print(
                "Zeichenfenster erst schließen! Mehrfaches Starten führt zu Instabilität. Am besten, du startest den Kernel neu!")
        ...
        self.schieberegler = self.Schieberegler(
            "Tempo", self.FPS, 1, 300, lambda: self.FENSTERBREITE - 100, lambda: self.FENSTERHOEHE - 50, self.fenster,
            self)
        ...
        if self.IstStandardfigur:
            ...
            for element in self.bausteine:
                if element[0] == "Rechteck":
                    x = int(round((self.maxVal * self.groesse / 50 - self.groesse) / 2 + self.groesse * element[
                        1] / 100 + self.groesse / 2))
                    y = int(round((self.maxVal * self.groesse / 50 - self.groesse) / 2 + self.groesse * element[
                        2] / 100 + self.groesse / 2))
                    breite = int(round(self.groesse * element[3] / 100))
                    hoehe = int(round(self.groesse * element[4] / 100))
                    farbeCodiert = element[5]
                    string = "pygame.draw.rect(self.image," + str(farbeCodiert) + ", (" + str(x) + "," + str(
                        y) + "," + str(breite) + ", " + str(hoehe) + "))"
                if element[0] == "Ellipse":
                    ...
                    hoehe = int(round(self.groesse * element[4] / 100))
                    farbeCodiert = element[5]
                    string = "pygame.draw.ellipse(self.image," + str(farbeCodiert) + ", (" + str(x) + "," + str(
                        y) + "," + str(breite) + ", " + str(hoehe) + "))"
                if element[0] == "Dreieck":
                    x1 = int(round((self.maxVal * self.groesse / 50 - self.groesse) / 2 + self.groesse * element[
                        1] / 100 + self.groesse / 2))
                    x2 = int(round((self.maxVal * self.groesse / 50 - self.groesse) / 2 + self.groesse * element[
                        2] / 100 + self.groesse / 2))
                    x3 = int(round((self.maxVal * self.groesse / 50 - self.groesse) / 2 + self.groesse * element[
                        3] / 100 + self.groesse / 2))
                    y1 = int(round((self.maxVal * self.groesse / 50 - self.groesse) / 2 + self.groesse * element[
                        4] / 100 + self.groesse / 2))
                    y2 = int(round((self.maxVal * self.groesse / 50 - self.groesse) / 2 + self.groesse * element[
                        5] / 100 + self.groesse / 2))
                    y3 = int(round((self.maxVal * self.groesse / 50 - self.groesse) / 2 + self.groesse * element[
                        6] / 100 + self.groesse / 2))
                    farbeCodiert = element[7]
                    string = "pygame.draw.polygon(self.image," + str(farbeCodiert) + ", [(" + str(x1) + "," + str(
                        y1) + "),(" + str(x2) + ", " + str(y2) + "),(" + str(x3) + "," + str(y3) + ")])"
